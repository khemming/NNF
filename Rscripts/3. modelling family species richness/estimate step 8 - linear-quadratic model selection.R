...
# quandratic model support if quadratic is better (delta == 0) or linear model has delta <8  
  ms_mat <- lq3 %>% 
            mutate(delta = as.numeric(delta)) %>%
            filter((model_type == "qm[[i]]" & delta == 0) | 
                   (model_type == "lm[[i]]" & between(delta, 0, 8))) %>%
            filter(family != "Asteraceae" | model_type == "lm[[i]]",         
                   family != "Chenopodiaceae" | model_type == "lm[[i]]",
                   
                   family != "Ericaceae" | model_type == "lm[[i]]",
                   family != "Euphorbiaceae" | model_type == "lm[[i]]",         
                   
                   family != "Malvaceae" | model_type == "lm[[i]]",
                   family != "Plantaginaceae" | model_type == "lm[[i]]",         
                   
                   family != "Poaceae_C3" | model_type == "lm[[i]]",      
                   family != "Scrophulariaceae" | model_type == "lm[[i]]",         
                   
                   family != "Solanaceae" | model_type == "lm[[i]]") %>%    
