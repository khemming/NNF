...
# required families
  names <- read.csv("Results/csv/summary statistics/study families.csv", stringsAsFactors = F)[1:19, 1]
...
# 3 pla -- Plantae (all families)
  ...
  inext_pp <- function(photosyn, stat) {
    ...
    # save raster
    rasterfile <- paste0("Results/rasters/iNEXT/", photosyn, "_", stat, ".grd")
    writeRaster(x[[7]],
                filename = rasterfile,
                overwrite = T)
    ...
    df <- cbind(records, 
                ...
                inext)
    head(df)
    csvfile <- paste0("Results/csv/iNEXT/", photosyn, "_", stat, ".csv")
    write.csv(df, csvfile)
