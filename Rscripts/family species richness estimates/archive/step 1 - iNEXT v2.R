...
# data ----------------------------------------------------------------------------
# australia 
  raster <- raster("Data files/Australia/Australia 100 km.grd")
  
# record data
  dat <- readRDS("Data files/ALA/master data/master records.RDS")[100000, ]
...
# assign each point in the dataframe to raster cell
  ...
  dat$cell <- raster::extract(raster, xy, na.rm = F)
...
# 3 pla -- Plantae (all families)
  inext_fam <- function(family_name, stat) {
    dat_fam <- dat %>%
               filter(family == family_name & status == stat) %>%
               mutate(species = factor(species))
  ...
  # save raster
    rasterfile <- paste0("Results/rasters/iNEXT/", family_name, ".", stat, ".grd")
    writeRaster(x[[7]],
                filename = rasterfile,
                overwrite = T)
  ...
  # save data frame  
    ...
    df <- cbind(records, 
                ...
                inext)
    head(df)
    csvfile <- paste0("Results/csv/iNEXT/", family_name, ".", stat, ".csv")
    write.csv(df, csvfile)
    
  }
  inext_pp <- function(photosyn, stat) {
    dat_pp <- dat %>%
      ungroup() %>%
    ...
    x <- inext(dat_pp, min.rec = 15, coverage = 0.8)  
  ...
  inext_pla <- function(stat) {
    pla <- dat %>%
      ungroup() %>%
      filter(status == stat) %>%
      mutate(species = factor(species))
    
    x <- inext(pla, min.rec = 15, coverage = 0.8)  
...
# Poaceae C3
  inext_pp("C3", "native")
  inext_pp("C3", "nonnative")

# Poaceae C4
  inext_pp("C4", "native")
  inext_pp("C4", "nonnative")
