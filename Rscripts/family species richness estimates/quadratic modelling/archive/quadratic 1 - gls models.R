...
# model methods to account for spatial autocorrelation
    model_e <- gls(q_form, data = fpv, correlation = corExp(form = ~long + lat, nugget=T) , na.action = na.omit, method = "ML")
    model_g <- gls(q_form, data = fpv, correlation = corGaus(form = ~long + lat, nugget=T), na.action = na.omit, method = "ML")
    model_s <- gls(q_form, data = fpv, correlation = corSpher(form = ~long + lat, nugget=T), na.action = na.omit, method = "ML")
    model_r <- gls(q_form, data = fpv, correlation = corRatio(form = ~long + lat, nugget=T), na.action = na.omit, method = "ML")
...
# Exp
  ...
  model_list[[x]] <- gls(q_form, correlation = corExp(form = ~long + lat, nugget=T), 
...
# Gaus
  ...
  fam <- fpv[, x]
  cor_str2[x, "run_method"]
  model_list[[x]] <- gls(q_form, correlation = corGaus(form = ~long + lat, nugget=T), 
                         na.action = na.omit, method = "ML", data = fpv)
  ci_list[[x]] <- data.frame(intervals(model_list[[x]], 0.95, which = "coef")$coef)
# Spher
  x <- 3
  fam <- fpv[, x]
  cor_str2[x, "run_method"]
  model_list[[x]] <- gls(q_form, correlation = corSpher(form = ~long + lat, nugget=T), 
                         na.action = na.omit, method = "ML", data = fpv)
  ci_list[[x]] <- data.frame(intervals(model_list[[x]], 0.95, which = "coef")$coef)
# Spher
  x <- 4
...
# Ratio
  ...
  fam <- fpv[, x]
  cor_str2[x, "run_method"]
  model_list[[x]] <- gls(q_form, correlation = corRatio(form = ~long + lat, nugget=T), 
                         na.action = na.omit, method = "ML", data = fpv)
  ci_list[[x]] <- data.frame(intervals(model_list[[x]], 0.95, which = "coef")$coef)
# Gaus
  x <- 6
...
# save ---------------------------------------------------------  
# Rdata
  save.image("Data files/Rdata/quadratic_data.RData")
