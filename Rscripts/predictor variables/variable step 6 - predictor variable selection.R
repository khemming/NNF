...
# data -----------------------------------------------------------------------
  dat <- read.csv("Results/csv/families and predictors/predictor variables 2538.csv", header = T) %>%
    select(-cell_id, -lat, -long, -cell_category, -proportion_cover)
  
# variable selection  
  vs <- dat %>% 
    dplyr::select(pcoldq, pwarmq, amt, ts, arid, th, hii)
  
# correlation matrix -------------------------------------------------------
  pcor <- round(cor(dat, method = "pearson"), 2)
  write.csv(pcor, "Results/csv/supplementary materials/predictor variable correlation matrix.csv")  
  
# reduced EVs on account of variable selection 
  pcor_vs <- round(cor(vs, method = "pearson"), 2)
  write.csv(pcor_vs, "Results/csv/supplementary materials/variable selection correlation matrix.csv")  
