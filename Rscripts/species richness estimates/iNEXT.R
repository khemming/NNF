...
# data ----------------------------------------------------------------------------
# australia 
  raster <- raster("Data files/Australia/aus 100 km v1.grd")
  
# family data
  dat.fam <- readRDS("Data files/ALA/master data/master family data.RDS") 
  
# Plantae data
  dat.pla <- readRDS("Data files/ALA/Plantae master data.RDS")
  
# required families
  names <- read.csv("Data files/ALA/summary statistics/family list.csv")
  names <- as.character(names$family)
...
# 1) fam -- family and status, 
# 2) pp -- Poaecae C3/C4
# 3 pla -- Plantae (all families)
  inext_run_fam <- function(fam, stat) {
    dat <- dat.fam %>%
      ungroup() %>%
  ...
  # save raster
    rasterfile <- paste0("Results/rasters/iNEXT/", fam, ".", stat, ".grd")
    writeRaster(x[[7]],
                filename = rasterfile,
                overwrite = T)
  ...
  # save data frame  
    ...
    df <- cbind(records, 
                ...
                inext)
    head(df)
    csvfile <- paste0("Results/csv/iNEXT/", fam, ".", stat, ".csv")
    write.csv(df, csvfile)
    
  }
  inext_run_pp <- function(photosyn, stat) {
    dat <- dat.fam %>%
      ungroup() %>%
    ...
    # save raster
    rasterfile <- paste0("Results/rasters/iNEXT/", photosyn, ".", stat, ".grd")
    writeRaster(x[[7]],
                filename = rasterfile,
                overwrite = T)
    ...
    df <- cbind(records, 
                ...
                inext)
    head(df)
    csvfile <- paste0("Results/csv/iNEXT/", photosyn, ".", stat, ".csv")
    write.csv(df, csvfile)
    
  }
  inext_run_pla <- function(stat) {
    dat <- dat.pla %>%
      ungroup() %>%
      filter(status == stat) %>%
      mutate(species = factor(species))
    ...
    # save raster
    rasterfile <- paste0("Results/rasters/iNEXT/Plantae.", stat, ".grd")
    writeRaster(x[[7]],
                filename = rasterfile,
                overwrite = T)
    ...
    df <- cbind(records, 
                ...
                inext)
    head(df)
    csvfile <- paste0("results/csv/iNEXT/Plantae.", stat, ".csv")
    write.csv(df, csvfile)
...
# loop for all groups ----------------------------------------------------------
  for (i in 1:length(names)) {
    ...
    inext_run_fam(names[i], "native")
    inext_run_fam(names[i], "non-native")
  }
  
  
# Poaceae C3
  inext_run_pp("C3", "native")
  inext_run_pp("C3", "non-native")

# Poaceae C4
  inext_run_pp("C4", "native")
  inext_run_pp("C4", "non-native")
  
# Plantae
  inext_run_pla("native")
  inext_run_pla("non-native")
