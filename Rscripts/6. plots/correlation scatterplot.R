...
# Fabaceae sans Acacia ----------------------------------------------------------
# data
  x <- getValues(raster("C:/Users/s436862/Dropbox/NNF/Results/Fabaceae sans Acacia/rasters/scaled/FNA_native_observed.grd"))
  y <- getValues(raster("C:/Users/s436862/Dropbox/NNF/Results/rasters/scaled/Fabaceae_nonnative_observed.grd"))
  
  dat <- data.frame(cbind(x, y))
  head(dat)

# labels
  title <- "FNA correlation"
  xlab <- "Native richness (scaled)"
  ylab <- "Nonnative richness (scaled)" 
# correlation
  cor <- sprintf("%.2f", round(cor(x, y, use = "complete.obs", method = "spearman"), 2))
...
# save
  save1 <- paste0("Results/correlation plots/", title, ".jpeg")
  save2 <- paste0("Results/major figures by family/Fabaceae sans Acacia/correlation.jpeg")
# run  
  corr_fun(dat, title, xlab, ylab, label, save1, save2)
