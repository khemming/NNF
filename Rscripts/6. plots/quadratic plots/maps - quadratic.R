...
# adding north and scale bars -----------------------------------------------
  pred_v10 <- function(raster, title, family, sr_breaks, sr_max, legend_title){
...
# note: version 9_po maintains scaled legend to 0 and 1
  ras_v9_po <- function(raster, title, family, sr_breaks, legend_title){
...
# potential by predicted native maps
  for (i in 1:length(names(q.po.stack))) {
    ...
    ras_v9_po(raster, title, family, sr_breaks, legend_title)
